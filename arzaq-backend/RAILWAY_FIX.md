...
### 2. Добавлен Dockerfile
Создан `Dockerfile` для корректного деплоя на Railway с:
- Установкой необходимых системных зависимостей (gcc, postgresql-client)
- Правильной обработкой переменной окружения PORT
- Созданием директорий для загрузок

### 3. Исправлен `app/routers/__init__.py`
