...
class Settings(BaseSettings):
    ...
    def CORS_ORIGINS(self) -> List[str]:
        ...
        origins = [
            self.FRONTEND_URL.rstrip('/'),  # Remove trailing slash if any
            self.FRONTEND_DEV_URL.rstrip('/'),
            "http://localhost:3000",  # Alternative dev port
            "http://localhost:5173",  # Vite default
        ]
        # Also add production Vercel domain explicitly
        if "vercel.app" in self.FRONTEND_URL:
            origins.append("https://arzaqmeal.vercel.app")
        return origins
