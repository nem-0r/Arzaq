...
async def create_order(
    ...
    **Note:** Order becomes PAID after successful payment via PayBox
    ...
    # ✅ IMPORTANT: Client pays ONLY subtotal (food price)
    # Platform fee (10%) is deducted from restaurant's revenue, not added to client's payment
    # Restaurant receives: subtotal * 0.9 (90%)
    # Platform receives: subtotal * 0.1 (10%)
    platform_fee = subtotal * 0.10  # For accounting purposes only
    total = subtotal  # ✅ Client pays ONLY for food, NO EXTRA FEE
    ...
    new_order = Order(
        ...
        subtotal=subtotal,
        platform_fee=platform_fee,  # Recorded for accounting, but not charged to client
        total=total,  # = subtotal (client pays only food price)
